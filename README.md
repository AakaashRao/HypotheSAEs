...
3. **Batch vs. live API calls**: All OpenAI calls default to the standard live API for low-latency workflows. When you need to annotate or embed large corpora, set `backend="batch"` (or `backend="auto"`) on `get_openai_embeddings`, `annotate`, `annotate_texts_with_concepts`, `generate_hypotheses`, or `evaluate_hypotheses` to route requests through [OpenAI Batch](https://platform.openai.com/docs/guides/batch). Batch jobs are slower but cheaper and bypass short-term rate limits. The batch cache directory defaults to `batch_cache/` (override with `HYPOTHESAES_BATCH_DIR`), and the switching threshold for `backend="auto"` can be tuned with `HYPOTHESAES_EMBED_BATCH_THRESHOLD`, `HYPOTHESAES_ANNOTATION_BATCH_THRESHOLD`, and `HYPOTHESAES_INTERPRETATION_BATCH_THRESHOLD`.
